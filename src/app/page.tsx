...
export default async function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50/30">
      ...
      {/* 1. HERO BLOCK (Полноэкранный баннер 90vh с аппаратным решением распорки) */}
      ...
      {/* 2. МЕДИЦИНСКИЕ ОТДЕЛЕНИЯ КЛИНИКИ (ИСПРАВЛЕНО: Отказоустойчивый мапинг векторных иконок Lucide по точным слагам СУБД) */}
        ...
        <div className="container mx-auto max-w-7xl px-6 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {!directions || directions.length === 0 ? (
              <div className="p-8 text-center bg-white rounded-3xl shadow-xs lg:col-span-4">
              ...
              directions.map((dir) => {
                ...
                // ИСПРАВЛЕНО: Строгая карта точных соответствий слагов из базы данных и монохромных иконок Lucide
                ...
                // Запасной дефолтный конфиг, если слаг из базы данных не совпал ни с одним ключом словаря
                const config = iconMap[currentSlug] || {
                  icon: <Activity className="w-4 h-4" />,
                  bg: "bg-slate-50 text-slate-600 group-hover:bg-slate-100",
                  status: "Амбулаторный прием",
                  desc: dir.seoDescription || "Специализированная диагностика, профилактика и эффективные методы амбулаторного лечения заболеваний отделения."
                };
                ...
                return (
                  <Link
                    ...
                    className="group bg-white p-6 rounded-3xl shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between min-h-[160px] relative overflow-hidden"
                  >
                    <div className="space-y-4">
                      {/* Верхний ряд: Заголовок и новая монохромная иконка */}
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {/* Контейнер адаптивной SVG-графики */}
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-200 shadow-3xs ${config.bg}`}>
                          {config.icon}
                        </div>
                      </div>
                    ...
                    {/* Подвал карточки с заменой стрелочки на тонкий шеврон */}
      ...
      {/* 3. ОБЪЕДИНЕННЫЙ МЕДИА-СЛАЙДЕР (ИСПРАВЛЕНО: Добавлены глубокие премиальные отступы py-24 и space-y-12) */}
      <section id="promos" className="py-24 bg-white overflow-hidden select-none">
        <div className="container mx-auto max-w-7xl px-6 space-y-12">
          ...
          {/* Интерактивный слайдер с внутренним отступом pb-6 для отдаления точек */}
          <div className="pb-6">
      ...
      {/* 2.6. ПОПУЛЯРНЫЕ УСЛУГИ И ЦЕНЫ (ИСПРАВЛЕНО: Лёгкая типографика, контрастные ссылки и глубокие отступы py-20) */}
      <section className="py-20 bg-slate-50/50 select-none">
        <div className="container mx-auto max-w-7xl px-6 space-y-10">

          {/* Заголовок секции: переведен на легкий естественный регистр, описание на токен */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2 max-w-2xl">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-600 select-none">
                Прейскурант
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {!popularServices || popularServices.length === 0 ? (
              <div className="p-8 text-center bg-white rounded-3xl shadow-xs lg:col-span-2">
              ...
              popularServices.map((service) => {
                ...
                return (
                  <div
                    key={service.id}
                    className="group bg-white p-5 rounded-2xl shadow-xs hover:shadow-md transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-none"
                  >
                    ...
                    <div className="space-y-1 flex-1">
                      ...
                      {/* ИСПРАВЛЕНО: Ссылки подробнее стали контрастными, глубокими и легко читаемыми */}
                      <Link
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 border-slate-50 pt-3 sm:pt-0 shrink-0">
                      ...
                      <a
                        href="#contacts"
                        className="px-5 h-8 bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-bold rounded-full shadow-2xs transition-all duration-150 flex items-center justify-center whitespace-nowrap"
                      >
                        Запись
                      </a>
                    </div>
      ...
      {/* 4. НАШИ ВРАЧИ (ИСПРАВЛЕНО: Премиальный смарт-слайдер гармошка с отсечением сестринского персонала и py-24) */}
      <section id="doctors" className="py-24 bg-slate-50/50 select-none">
        <div className="container mx-auto max-w-7xl px-6 space-y-12">
          ...
          {/* Интеграция нашего интерактивного 3-позиционного слайдера врачей */}
          <DoctorsSlider doctors={doctors} />

        </div>
      </section>

      {/* 5. МЕДИАЦЕНТР (ИСПРАВЛЕНО: Объединенный блок переведен на легкую журнальную типографику) */}
      <section className="py-20 bg-white">
        <div className="container mx-auto max-w-7xl px-6 space-y-10">

          {/* ИСПРАВЛЕНО: Заголовок Медиацентра переведен на чистый естественный регистр font-bold */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2 max-w-2xl">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-600 select-none">
          ...
          {/* Сетка статей Медиацентра */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts && posts.slice(0, 3).map((post) => (
              <div
                key={post.id}
                className="group flex flex-col justify-between bg-slate-50/40 p-6 rounded-3xl shadow-xs hover:shadow-md transition-all duration-200"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-[11px] font-bold text-slate-400">
                    <span className="px-2.5 py-0.5 rounded-md bg-blue-50 text-blue-600 uppercase tracking-wider text-[9px] font-black shadow-3xs">
                      Статья
                    </span>
                    <span>
                ...
                <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-blue-600">
                  <Link href={`/blog/${post.slug}`} className="hover:text-blue-700 group-hover:translate-x-0.5 transition-transform flex items-center gap-1">
                    Читать материал →
                  </Link>
                </div>
              </div>
      ...
      {/* 4.5. НОВОСТИ КЛИНИКИ (ИСПРАВЛЕНО: Лёгкая типографика, контрастные ссылки и глубокие отступы py-20) */}
      <section id="news" className="py-20 bg-white select-none">
        <div className="container mx-auto max-w-7xl px-6 space-y-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            {/* Кнопка перехода в архив новостей */}
            <Link
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!newsList || newsList.length === 0 ? (
              <div className="p-8 text-center bg-slate-50/50 rounded-3xl shadow-xs lg:col-span-3">
              ...
              newsList.slice(0, 3).map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="group bg-slate-50/40 p-6 rounded-3xl shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between min-h-[220px] border-none"
                  >
                    <div className="space-y-4">
                      {/* Бейджи и дата */}
                      ...
                      {/* Заголовок и анонс текста */}
                      <div className="space-y-2">
                        <h3 className="text-sm font-bold text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
                          {item.title}
                        </h3>
                        {/* ИСПРАВЛЕНО: Текст анонса новости теперь жестко управляется через центральный токен medical-description */}
                        <p className="medical-description line-clamp-3">
                          {item.content}
                        </p>
                      </div>
                    ...
                    {/* Подвал карточки с контрастной интерактивной ссылкой */}
                    <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-end text-[11px] font-bold">
                      {/* Поскольку детальные страницы новостей ещё могут быть не проработаны, временно ведём на якорную заглушку #contacts по аналогии с акциями */}
                      <Link
                        href="#contacts"
      ...
      {/* 4.5. ОТЗЫВЫ ПАЦИЕНТОВ (Парящий блок без рамок с чистым архитектурным подключением MyReviews) */}
      <section id="reviews" className="py-20 bg-white scroll-mt-16">
        <div className="container mx-auto max-w-7xl px-6 space-y-10">
          ...
          {/* Вызов нашего изолированного клиентского компонента */}
          <MyReviewsWidget />

        </div>
      </section>

      {/* 5. КОНТАКТЫ И СХЕМА ПРОЕЗДА (Финальный блок на главной — без рамок, с интеграцией Яндекс.Виджета) */}
      <section id="contacts" className="py-20 bg-slate-50/50 scroll-mt-16">
        <div className="container mx-auto max-w-7xl px-6 space-y-10">
          ...
          {/* ИНТЕГРАЦИЯ ИНТЕРАКТИВНОГО ВИДЖЕТА ЯНДЕКС.КАРТ БЕЗ РАМОК НА ПАРЯЩЕЙ ТЕНИ */}
          <div className="w-full bg-white rounded-3xl p-2 shadow-xs transition-shadow duration-300 hover:shadow-md overflow-hidden">
            <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: "460px", minHeight: "460px" }}>
              ...
              <iframe
                src="https://yandex.ru/map-widget/v1/org/klinika_verbenkina/11589215365/?ll=37.148288%2C55.965325&z=16"
                width="100%"
                height="100%"
          ...
          {/* Юридический блок внизу карты без рамок с легкой тенью */}
      ...
      {/* 7. ОБЯЗАТЕЛЬНЫЙ YMYL ДИСКЛЕЙМЕР МИНЗДРАВА */}
